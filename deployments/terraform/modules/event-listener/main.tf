...
resource "aws_lambda_function" "applicant_atlas_event_listener" {
  ...
  image_uri     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${aws_ecr_repository.applicant_atlas_event_listener_lambda.name}:latest"
